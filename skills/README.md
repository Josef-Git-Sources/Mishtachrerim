...
\### 1. docs-drift-reviewer

\*\*Type:\*\* review-only  

\*\*Purpose:\*\* detect implementation-to-doc drift and relevant doc inconsistency against the project's source of truth.
...
\### 2. post-batch-architecture-reviewer
...
\### 3. cloud-build-and-lint-gatekeeper
...
\### 4. strict-rtl-hebrew-ui-inspector
...
\### 5. app-router-seo-and-metadata-auditor
...
\### 6. supabase-schema-to-type-sync
...
3\. `strict-rtl-hebrew-ui-inspector` \*(when UI changed)\*

4\. `app-router-seo-and-metadata-auditor` \*(when public SEO routes changed)\*

5\. `supabase-schema-to-type-sync` \*(when schema/types/data access changed)\*

6\. `cloud-build-and-lint-gatekeeper`
